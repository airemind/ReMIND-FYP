...
const Dashboard = () => {
  ...
  const createNewChat = async () => {
    try {
      ...
      navigate(
        `/dashboard/chat/${normalizedChat.id}?name=${encodeURIComponent(normalizedChat.title || "New Chat")}`,
      );
    } catch (error) {
      console.error(error);
  ...
  const sendMessage = async (payload) => {
    try {
      ...
      const imageAttachment = payload.files?.find(
        (f) => f.fileType === "image",
      );

      const audioAttachment = payload.files?.find(
        (f) => f.fileType === "audio",
      );
      ...
      const assistantMessage = {
        ...
        enhancedImage:
          response?.image_ai?.enhanced_url ||
          response?.image_ai?.original_url ||
          "",
  ...
  const handleEnhanceImage = async (messageId) => {
    try {
      ...
      const targetMessage = currentChat?.messages.find(
          ...
          messages: chat.messages.map((msg) =>
            msg.id === messageId
              ? {
                  ...
                  enhancedDownloadUrl:
                    result?.enhanced_url || result?.enhanced_image || null,
                }
              : msg,
          ),
        })),
...
========================= */
  ...
  const handleChatSelect = (id) => {
    ...
    navigate(
      `/dashboard/chat/${id}?name=${encodeURIComponent(
    ...
    <div className="dashboard">
      ...
      <div className="main-area">
        ...
        <div className="dashboard-disclaimer">
          ReMIND can make mistakes. Verify important information and uploaded
          content before relying on AI-generated responses.
        </div>
      </div>
    </div>
  );
