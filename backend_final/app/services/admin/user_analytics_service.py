...
def get_user_analytics(
    ...
    logged_in_users = db.query(User).filter(
        User.is_logged_in == True
    ).count()

    logged_out_users = db.query(User).filter(
        User.is_logged_in == False
    ).count()

    logged_in_active_users = db.query(User).filter(
        User.is_logged_in == True,
        User.is_active == True
    ).count()

    logged_in_inactive_users = db.query(User).filter(
        User.is_logged_in == True,
        User.is_active == False
    ).count()

    logged_out_active_users = db.query(User).filter(
        User.is_logged_in == False,
        User.is_active == True
    ).count()

    logged_out_inactive_users = db.query(User).filter(
        User.is_logged_in == False,
        User.is_active == False
    ).count()
